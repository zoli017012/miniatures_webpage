...
async function applyPointFilter(minVal, maxVal) {
    ...
    await loadCardsByState(chosenState, chosenFraction, chosenRarity, chosenMinPoint, chosenMaxPoint);
}
...
function sortCharsByPointAsc(){
    ...
    loadCardsByState('All', 'Összes', 'Összes', chosenMinPoint, chosenMaxPoint)
}
...
async function loadCardsByState(state, fraction, skills, minPoint, maxPoint) {
    container.innerHTML = '';
    for(let i = 0; i < characters.length; i++){
        ...
        if ((card_to_be_loaded.points >= minPoint) && (card_to_be_loaded.points <= maxPoint) && (card_to_be_loaded.set_name == chosenSet || chosenSet == 'Összes') && (card_to_be_loaded.fraction == chosenFraction || chosenFraction == 'Összes') && include_checker(card_to_be_loaded.skill_set, chosen_skills) && exclude_checker(card_to_be_loaded.skill_set, excluded_skills) || chosenState == 'All'){
                        ...
                        <div class="modal-card">
                            ...
                            <div class="card-controls">
                                ...
                                <div class="control-group">
                                    <label>Csapat:</label>
                                    <select class="state-select" id="char-to-team" onchange="addCharToTeam('${card_info.id}', '${card_info.points}', '${card_info.fraction}' ,this.value)">
...
async function init(){
    await loadChars()
    await loadCardsByState("All", "Összes", "Összes", 0, 115)
    await fillSideList()
...
async function fillSideList(){
    ...
    for (let i = 0; i < fractions.length; i++) {
        ...
        element.addEventListener('click', (event) => {
            ...
            loadCardsByState(chosenState, chosenFraction, chosenRarity, chosenMinPoint, chosenMaxPoint)
        })
...
async function loadTeam(team_name) {
    chosenTeamName = team_name
    ...
    const chosenTeam = team_data.find(x => x.name == team_name)
                ...
                modalContent.innerHTML = `
                        <div class="modal-card">
                            ...
                            <div class="card-controls">
                                <!-- Állapot választó -->
                                <div class="control-group">
                                    <button onclick="removeCharFromTeam('${card_info.id}', '${team_name}')" class="sw-btn">Törlés a csapatból</button>
                                </div>
                            </div>
                        </div>
                    `;
...
async function changePointCriteria(p) {
    ...
    loadCardsByState(chosenState, chosenFraction, chosenRarity, minPoint, maxPoint)
